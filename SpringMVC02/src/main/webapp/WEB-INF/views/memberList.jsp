...
/* 		var html = $("#collapse1 .panel-body").html(); 
		alert(html); */		
	    $.ajax({
	    	url : "<c:url value='/memberAjaxList.do' />", /* 요청을 보낼 컨트롤러 지정 */
	    	type : "GET",
	    	dataType : "json", /* 컨트롤러(서버)로부터 응답을 json형태로 받겠다. */
	    	success : memList, /* 컨트롤러에 요청 성공 시 응답에 대한 결과데이터를 아래의 callback함수에서 받아줌. */
	    	error : /* error */function() { alert("컨트롤러로 요청 실패 error"); } /* 컨트롤러에 요청 실패 시 호출되는 함수. */
	    });
	}
	
/* json형태(배열) : [{회원1}, {회원2}, ...., {회원n}] 
...
/* callback함수 */
	function memList(members) {
		/* callback함수까지 데이터가 잘 넘어왔는지 브라우저에서 확인 가능.   */
		console.log(members);
...
/*JQuery 반복문 list(json)				object  */ /*json형태 : [{회원1}, {회원2}, ...., {회원n}]*/
        ...
        html += "</table>";
        /* 이 부분("#collapse1 .panel-body")을 위의 html내용물로 넣어준다. */
