...
//현재, DAO는 mapper.xml과 연결되어있다! -> 스프링에서 DAO를 인터페이스(MapperInterface)로 교체.
//MapperInterface(메서드)와 mapper.xml(id) 연결 ! -> xml파일도 사용하지 않고, 메서드에 어노테이션으로 교체.
//간단한 쿼리는 어노테이션, 복잡한 쿼리는 xml파일 사용할 것.

//@(어노테이션 : 전처리) -> 객체가 생성되어 SpringContainer에서 관리를 해준다. DI(의존성 주입) : 의존관계 느슨하게..
// component scan에 의해 인식.
@Repository
...
//	컨테이너에 bean으로 등록되어있는 factory 가져다 쓴다.
	@Autowired
